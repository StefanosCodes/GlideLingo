...
- Add a repository-pinned @playwright/test development dependency through npm and commit the
  package-lock change.
...
- Use a Playwright webServer entry that calls the repository's Expo web script and waits on a
  deterministic URL. In multi-worktree use, default to an isolated port and do not reuse an
  unrelated existing server.
