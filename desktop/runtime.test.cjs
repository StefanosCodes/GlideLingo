...
test('supported app dispatch preserves the active loopback renderer for auth callbacks', () => {
  const loaded = [];

  assert.equal(dispatchSupportedAppUrl('glidelingo://app/sso-callback?state=opaque', {
    activateWindow() {},
    hasWindow: () => true,
    loadRendererUrl: (url) => loaded.push(url),
