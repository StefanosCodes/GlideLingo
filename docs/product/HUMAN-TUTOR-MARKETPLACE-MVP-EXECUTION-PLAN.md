...
| 7. Learning bridge | Complete | `e604223df683da87b76c527fad2ba564666ad812` | `2d46f8979a9b3ce6b3a91b95d5092198369cf306` | `npm run verify:full-stack` passed at the immutable feature head with 130 client tests, 154 API unit tests, 31 PostgreSQL integration tests, 73 desktop tests, 15 private-tutor tests, Expo Doctor 21/21, and successful web export of the separately protected learning-context route; focused coverage proves explicit booking-scoped grant, idempotent revoke, hidden context without consent or after revoke/expiry, preserved consent audit history, participant and guessed-ID isolation, reschedule-aware seven-day tutor access, learner-selected no-course context, bounded course/capability/review projections, assigned-tutor-only follow-up, free-text recommendations, runtime denial of consent-audit mutation, default-off flags, and a UI/API contract with no mastery, XP, entitlement, unlock, or official-evidence write path | Authenticated learner/tutor browser or Electron account-switch evidence remains external because this worktree has no valid Clerk development identities. The learning-context flag stays off; retention/deletion duration, migration application, deployment, and activation require privacy/operator approval and no provider or production state was mutated |
| 8. Hardening/pilot readiness | Complete | `eeee904f07c934ac367d1ad8cc9afa6cf875ad0b` | `ffc4a05063976b60762aafe0ee6ac77728da664d` | `npm run verify:full-stack` passed at the immutable feature head with 131 client tests, 161 API unit tests, 31 PostgreSQL integration tests, 73 desktop tests, 15 private-tutor tests, Expo Doctor 21/21, and successful export of 45 web routes including capability-scoped booking operations; focused evidence proves the independent default-off acquisition rollback boundary blocks discovery, new pre-booking conversations, and checkout while preserving tutor administration and existing booking/support routes; a runnable lease-aware money worker handles graceful shutdown and safe structured outcomes; runtime-role tests deny schema creation and mutation/deletion of audit, transition, webhook, schedule-revision, consent, and money ledgers; the integrated runbook records migration order, credential provenance without payloads, monitoring/alerts, incident recovery, rollback, readiness separation, and every unexecuted external acceptance row | Repository code readiness is complete, but integration, operational, and enablement readiness are not: real Stripe/Google/Clerk/email journeys, actual browser/Electron/iOS/Android authenticated account-switch journeys, production account/secret provenance, migration application, deployed money/notification/reminder workers and maintenance, dashboards/alert routing, named support/operators, legal/privacy/terms/tax/contractor/store approval, rollback rehearsal, and one approved allowlisted production transaction remain explicit external gates. All flags remain off; no vendor, database, deployment, money, or activation state was mutated and no live evidence is claimed |
| 8R. Independent-review remediation | Complete | `6874309c29fda85408bbc8ae8eddca088b6d1a8d` | `f9c0f4f21dd14e57743ba670592d94ff1e7568ee` | Independent principal review findings were corrected in implementation commit `0821ef047b20e9a78c2abcaefa1ed3f772d06ad3`, then current `origin/main` at `466f8e21eeb741a5f8bc07d069b099a28d4319aa` was merged without rewriting history. `npm run verify:full-stack` passed at the immutable integration head with 151 client tests, 189 API unit tests, 34 PostgreSQL integration tests, 81 desktop tests, 15 private-tutor tests, Expo Doctor 21/21, and a successful 46-route web export. Focused evidence now covers late-payment/expired-hold compensation without inventory confirmation, durable composite-job processing and terminal retries, buffer-aware database overlap exclusion, reschedule validation against current schedule/calendar/profile state, exact-deadline dispute/transfer ordering, an independent payout-execution kill switch, lossless availability editing and timezone invalidation, discovery pagination beyond the first bounded scan, minimum-rating/duration filters, capability-scoped review moderation, and operator recovery of ambiguous money states. | Repository code and deterministic integration readiness are complete. Live Stripe/Google/Clerk/email evidence, authenticated browser/Electron/iOS/Android two-role journeys, production credential/account provenance, migration application, worker deployment, alert routing, named operators, legal/privacy/accounting/store approvals, rollback rehearsal, provider-sandbox mutation, an allowlisted live transaction, and flag enablement remain explicit external gates. All marketplace and payout-execution flags remain off; no live provider, shared database, deployment, money, or feature-enablement state was mutated. |
| 8F. Final activation-boundary review | Complete | `b0d7be3ad340a9fcb484ae56912e9e33b2286a9d` | `3792137908f814de95107b10878e3b111ff6ccb4` | A renewed principal review identified one final staged-activation defect: the shared durable worker required commerce even though it also owns independently enabled messaging and Google-calendar queues. The worker now starts only under the base Marketplace flag, skips commerce-owned expiry/reconciliation/money/reminder queues while commerce is disabled, and continues enabled notification, calendar-refresh, and retention queues. `npm run api:lint`, `npm run api:typecheck`, and all six focused worker tests passed at the immutable correction head, including explicit base-on/commerce-off queue-order coverage. | Deployment and live adapter evidence remain external; this correction changes no default flag value and permits no provider or database mutation while its child capability remains disabled. Full repository gates are rerun at the subsequent documentation-only head before PR publication. |
| 8G. Final review convergence | Complete | `691d6c4a8a6bcc1d836a677c58a0a7be0c1a7ce7` | `d9e6f2b5d07c376afe07309d33a3e8107f4d0485` | A second independent principal review signed off with no remaining P0–P2 findings after exact-evidence concurrent checkout attachment was made replay-safe, discovery cursors were moved to immutable tutor IDs, synchronous client guards closed same-tick availability/calendar/checkout races, and the migration `014` cardinality change was disclosed. At the immutable correction head, all 76 marketplace client tests, 49 marketplace backend unit tests, and the two focused PostgreSQL concurrency/cursor regressions passed; `npm run api:lint`, `npm run api:typecheck`, `npx tsc --noEmit`, and `git diff --check` also passed. | Full named repository gates are rerun at the subsequent documentation-only head before PR publication. Live Stripe/Google/Clerk/email evidence, authenticated multi-role device journeys, migration application, deployment, legal/accounting approvals, and activation remain explicit external gates; all flags remain off. |
| 8H. CI clock convergence | Complete | `bccc56fd30c47133acabc11e27fdae2f9f3c7233` | `7ad49d6c3011161feebcad1837803913121ea3f2` | PR #63 current-head CI exposed a mixed-clock race in notification-job availability: PostgreSQL insertion time could be later than the server-injected event time used by the worker claim. The failure was reproduced deterministically with a one-minute skew, and message notification jobs now persist `available_at`, `created_at`, and `updated_at` from the same server-owned event clock. The focused regression passed, all 38 PostgreSQL integration tests passed, `npm run api:lint`, `npm run api:typecheck`, and `git diff --check` passed, and an independent principal reviewer signed off with no correctness, security, retry, or concurrency findings. | Full named repository gates are rerun at the subsequent documentation-only head, then the branch is pushed and replacement current-head CI must pass. Live provider journeys, migration application, deployment, legal/accounting approvals, and activation remain explicit external gates; all flags remain off. |
