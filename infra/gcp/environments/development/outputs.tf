...
output "revenuecat_secret_containers" {
  description = "Development RevenueCat configuration containers; contains names only, never values."
  value = {
    for key, secret in google_secret_manager_secret.revenuecat : key => secret.secret_id
  }
}
