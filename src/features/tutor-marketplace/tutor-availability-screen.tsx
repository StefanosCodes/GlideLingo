  ...
  return <ScreenFrame testID="tutor-availability-screen"><View style={styles.header}><ThemedText type="eyebrow" themeColor="textSecondary">TUTOR AVAILABILITY</ThemedText><ThemedText type="display">Set dependable weekly hours.</ThemedText><ThemedText type="body" themeColor="textSecondary">Times use {state.availability.timeZone}. Lead time and lesson buffers are enforced by the server.</ThemedText></View>
    <GlideSurface padding="roomy" style={styles.card}>
      ...
      {rules.map((rule, index) => <GlideSurface key={`${index}-${rule.weekday}-${rule.effectiveFrom}`} padding="regular" style={styles.nested} variant="tinted">
        ...
        <Field label={`Rule ${index + 1} effective until (optional YYYY-MM-DD)`} onChangeText={(value) => setRules((current) => current.map((item, itemIndex) => itemIndex === index ? { ...item, effectiveUntil: value.trim() || null } : item))} value={rule.effectiveUntil ?? ''} />
      ...
      <GlideButton disabled={saving || rules.length === 0 || rules.some((rule) => !Number.isInteger(rule.weekday) || rule.weekday < 0 || rule.weekday > 6 || !/^\d{2}:\d{2}$/.test(rule.startLocal) || !/^\d{2}:\d{2}$/.test(rule.endLocal) || !/^\d{4}-\d{2}-\d{2}$/.test(rule.effectiveFrom) || (rule.effectiveUntil !== null && (!/^\d{4}-\d{2}-\d{2}$/.test(rule.effectiveUntil) || rule.effectiveUntil < rule.effectiveFrom))) || !Number.isInteger(Number(leadTime)) || !Number.isInteger(Number(bufferBefore)) || !Number.isInteger(Number(bufferAfter))} label={saving ? 'Saving availability…' : 'Save availability'} onPress={() => void save()} />
    </GlideSurface>
